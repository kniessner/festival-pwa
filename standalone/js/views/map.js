...
export function renderMap(container) {
    ...
    // The header on this page is not a fixed height (logo + "FESTIVAL MAP"
    // title + language switcher stack differently on phone vs desktop),
    // so hard-coding `calc(100vh - Xpx)` in CSS is fragile. Measure the
    // stage's y-offset once mounted and size it to fill the remaining
    // viewport. Also refresh on resize / orientation change.
    // Shared with attachPinchZoom — sizeStage() has to know if a gesture
    // is in progress so it can skip mid-pinch layout thrash on iOS.
    const gestureState = { active: false };

    const sizeStage = () => {
        // Don't re-lay-out the stage mid-gesture — on iOS the address bar
        // hides during pinch and fires `resize`, which would otherwise
        // recompute the object-fit image size right under the user's
        // fingers and cause a visible jump.
    ...
    // Detach listeners from the previous map-view mount, if any, so
    // repeated renderMap() calls don't stack window listeners.
