...
if [ -f "$SW_FILE" ]; then
    NEW_CACHE="bucht-standalone-v$(date +%s)"
    sed -i.bak "s/const CACHE_NAME = '[^']*'/const CACHE_NAME = '$NEW_CACHE'/" "$SW_FILE"
    rm -f "$SW_FILE.bak"
    ok "Cache renamed to $NEW_CACHE"
    echo "   ✅ Cache renamed to $NEW_CACHE"
else
